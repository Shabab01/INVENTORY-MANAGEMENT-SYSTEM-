...
-- To ensure data integrity, add foreign key constraints between Products.CategoryID and Categories.CategoryID.

-- Add foreign key constraint to ensure each product belongs to a valid category
ALTER TABLE Product
...
-- 2. Data Validation
-- To include data validation checks, you can add constraints on columns to ensure unit prices are positive and quantities sold are non-negative.

-- Ensure Unit_price is positive and Units_instock is non-negative
ALTER TABLE Product
MODIFY COLUMN Unit_price DECIMAL(10, 2) CHECK (Unit_price > 0),
MODIFY COLUMN Units_instock INT CHECK (Units_instock >= 0);

-- Ensure QuantitySold in Sales table is non-negative
ALTER TABLE Sales
MODIFY COLUMN QuantitySold INT CHECK (QuantitySold >= 0);

-- Basic Reports
-- Total Sales by Product
-- To calculate the total quantity and value of sales for each product.
...
-- To calculate total sales by category.
...
-- This report shows the current inventory status for each product.

SELECT 
    Product.Product_Name,
    Product.Units_instock AS Current_Inventory
FROM 
    Product
ORDER BY 
    Product.Units_instock DESC;
